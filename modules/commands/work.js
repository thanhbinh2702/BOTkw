...
module.exports.run = async ({ event, api, Currencies }) => {
    ...
    if (typeof data !== "undefined" && cooldown - (Date.now() - data.workTime) > 0) {
        ...
        const job = [
            ...
            "ăn dái trâu",
            "làm chó 24h",
            "Bạn bị Đức Bo địt, đéo nhận được đồng nào."
        ];
        const amount = Math.floor(Math.random() * 600);
        let jobb = job[Math.floor(Math.random() * job.length)];
        if (jobb == "Bạn bị Đức Bo địt, đéo nhận được đồng nào.") {
          return api.sendMessage(jobb, threadID, messageID);
        } else {
          return api.sendMessage(`Bạn ${jobb} và đã nhận được số tiền là: ${amount} coins`, threadID, async () => {
